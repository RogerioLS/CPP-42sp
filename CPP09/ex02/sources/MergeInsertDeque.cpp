...
/*                                                                            */
/*                                                        :::      ::::::::   */
/*   MergeInsertDeque.cpp                               :+:      :+:    :+:   */
/*                                                    +:+ +:+         +:+     */
/*   By: codespace <codespace@student.42.fr>        +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/04/13 18:05:43 by codespace         #+#    #+#             */
/*   Updated: 2025/04/13 18:27:30 by codespace        ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/**
* @brief Sorts a deque of integers using the Merge-Insert Sort algorithm.
* 
* This function implements a hybrid sorting algorithm that combines merge sort 
* and insertion sort, also known as the Ford-Johnson algorithm. It uses the
* same approach as the vector implementation but with deque-specific operations.
* 
* @param arr A reference to the deque of integers to be sorted.
* 
* @details
...
* 
* @warning The input deque is modified in place.
*/
void PmergeMe::mergeInsertSortDeque(std::deque<int>& arr)
{
    // Base case: if the deque has 1 or fewer elements, it's already sorted
    if (arr.size() == 1)
    {
        debugPrint(GREEN + std::string("Deque is already sorted (size <= 1)") + RESET);
        return;
    }
    ...
    // Handle odd element if deque size is odd
    ...
    // Add the first pair's elements to initialize the result
    if (!pairs.empty())
    {
        result.push_back(pairs[0].second); // The smaller element of the first pair
        result.push_back(largerElements[0]); // The larger element of the first pair (now sorted)
    ...
    // Pre-compute Jacobsthal numbers
    int j1 = 1, j2 = 3;
